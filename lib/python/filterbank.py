...
def create_filterbank_file(outfn, header, spectra=None, nbits=8, \
    ...
    if spectra is not None:
        ...
        if nbits==1: # added along with pack_1bit. 25.1.2017
            #spectra = pack_1bit(spectra)
            pack_1bit(spectra.flatten()).astype(dtype).tofile(outfile) # like this?
        else:
            spectra.flatten().astype(dtype).tofile(outfile)
    outfile.close()
...
def pack_1bit(data): # re-pack 1 bit data. added 25.1.2017
    packed = np.zeros(data.shape[0],dtype='uint8')
    for ii in range(8):
        packed[ii::8] = np.left_shift(data[ii::8],ii)
    tupacked = np.zeros(data.shape[0]/8, dtype='uint8')
    for i in range(len(tupacked)):
        tupacked[i] = sum(packed[8*i : 8*i + 8])
    return tupacked
